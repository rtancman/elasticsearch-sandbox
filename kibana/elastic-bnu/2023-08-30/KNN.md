...
POST _ml/trained_models/sentence-transformers__msmarco-minilm-l-12-v3/_infer
{
  "docs": [
    {
      "text_field": "eu vou no meetup de elastic em blumenau."
    }
  ]
}
...
### spark-shell para gerar um dataset qualquer
- https://www.rtancman.com.br/elasticsearch/elasticsearch-spark-ingestao-de-dados.html
...
val df = spark.read.option("header",true).options(Map("delimiter"->";")).csv("/Users/rtancman/Downloads/Spotify_Dataset_V3.csv")

val df2 = df.select(concat(df("Artists"),lit(" "),df("Title"),lit(" "),df("Nationality")).as("text"))
df2.show()

df2.write.option("header","false").csv("/Users/rtancman/Downloads/elastic-bnu-texts.csv")
```
